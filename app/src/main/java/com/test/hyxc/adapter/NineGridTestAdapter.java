...
public class NineGridTestAdapter extends BaseAdapter implements View.OnClickListener {
    ...
    //标志变量
    boolean bol = false;
    public NineGridTestAdapter(Context context,String pageType,Boolean  bol) {
        mContext = context;
...
*/
        ...
        holder.iv_more.setOnClickListener(NineGridTestAdapter.this);
        if(pageType.equals("myWork") && bol){
            holder.iv_more.setVisibility(View.VISIBLE);
        }
        if(pageType.equals("islandWork") && bol) { //如果是海岛管理员 则显示
            holder.iv_more.setVisibility(View.VISIBLE);
        }
        return convertView;
    }
