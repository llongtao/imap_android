...
public class ChangeActActivity extends BaseActivity{
    //oss
    private OssConfig ossConfig=null;
    private OSS oss;
    private String bucket="yourbucket";
    private final static String imgEndpoint = "";
    String url="";
    //表征上传oss是否异常
    ...
    ImageView iv_cover_change,iv_select_cover;
    Button btn_publish;
    //时间选择器
    ...
    protected int getLayoutID() {
        return R.layout.imap_act_change;
    }
    @Override
    ...
    protected void initView() throws Exception {
        ...
        iv_cover_change=f(R.id.iv_cover_change);
        ...
        //如果是修改活动信息
        if(getIntent().hasExtra("activityShow")){
            activityShow =(ActivityShow) getIntent().getSerializableExtra("activityShow");
            tv_island_name2.setText(activityShow.getAct_island_name());
            et_title.setText(activityShow.getAct_title());
            et_act_text.setText(activityShow.getAct_text());
            SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            String startTime = sdf.format(new Date(activityShow.getAct_time().getTime()));
            date1.setTime(activityShow.getAct_time().getTime());
            ...
            String endTime = sdf.format(new Date(activityShow.getAct_end_time().getTime()));
            date2.setTime(activityShow.getAct_end_time().getTime());
            end_year=endTime.substring(0,4);
            end_month=endTime.substring(5,7);
            end_day=endTime.substring(8,10);
            end_hour=endTime.substring(11,13);
            end_minute=endTime.substring(14,16);
            tv_end_time2.setText(endTime);
            makeSureProvince(activityShow.getAct_city(),activityShow.getAct_district()); //省市区
            et_address.setText(activityShow.getAct_address());
            if(activityShow.getAct_island_university()!=null){
                university_id=activityShow.getAct_island_university();
                makeSureUniversity(activityShow.getAct_island_university());
            }
            /*if(activityShow.getAct_university_name()!=null && !"".equals(activityShow.getAct_university_name())){
                tv_uni_text.setText(activityShow.getAct_university_name());
                university=activityShow.getAct_university_name();
                university_id = activityShow.getAct_island_university();
            }*/
            if(activityShow.getResourcesList()!=null && activityShow.getResourcesList().size()>0) {
                Glide.with(ChangeActActivity.this).load(url + activityShow.getResourcesList().get(0).getAct_resource_url())
                        .asBitmap()
                        .error(R.mipmap.error)
                        .placeholder(R.mipmap.error)
                        .thumbnail(0.1f)
                        .diskCacheStrategy(DiskCacheStrategy.ALL)
                        .into(iv_select_cover);
            }
        }

        //弹出时间选择器选择生日
        pvTime= new TimePickerView.Builder(ChangeActActivity.this, new TimePickerView.OnTimeSelectListener() {
            @Override
        ...
        //弹出时间选择器选择生日
        pvTimeEnd= new TimePickerView.Builder(ChangeActActivity.this, new TimePickerView.OnTimeSelectListener() {
            @Override
            public void onTimeSelect(Date date, View v) {
                date2 = date;
                SimpleDateFormat sdf=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        ...
        ll_location.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                intent.setClass(ChangeActActivity.this, SelectProvinceActivity.class);
        ...
        ll_uni.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                intent1.setClass(ChangeActActivity.this, com.test.hyxc.page.university.SelectProvinceActivity.class);
                startActivityForResult(intent1, REQUEST_PRO_CITY_UNIVERSITY);
            }
        });
        iv_cover_change.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                gallery();
            }
    ...
    public void  tryPublish() throws Exception {
       ...
       /* else if(selectCover==null||selectCover.equals("")){
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    loadingDialog=new LoadingDialog(ChangeActActivity.this);
                    loadingDialog.show();
                }
            });
            new Thread(new Runnable() {
                ...
                public void run() {
                    String remoteUrl = "";
                    if(selectCover!=null && !"".equals(selectCover)){
                        //尝试上传图片
                        String localUrl=selectCover;
                        remoteUrl="activity/actis_"+activityShow.getAct_island()+"_"+ localUrl.substring(localUrl.lastIndexOf("/")+1,localUrl.lastIndexOf("."))+".jpg";
                        // 构造上传请求。
                        ...
                        map.put("act_linkman",String.valueOf(activityShow.getAct_linkman()));
                        ...
                        if(!"".equals(remoteUrl)&& selectCover!=null && !"".equals(selectCover)){
                            ...
                            netUtils.postDataAsynToNetToken(AsyncHttpHelper.URL_ACTIVITY_BASE + "/changeActivity",token, map, new NetUtils.MyNetCall() {
                                ...
                                public void success(Call call, Response response) throws IOException,JSONException {
                                    ...
                                    if(json.has("tokenState")&&json.getString("tokenState").equals("tokenTimeout")){
                                        startActivity(new Intent(ChangeActActivity.this, LoginActivity.class));
                                        overridePendingTransition(0,R.anim.imap_fade_out_right);
                                        finish();
                                    }else {
                                        if(json.has("changeState")&&json.get("changeState").equals("success")){
                                            runOnUiThread(new Runnable() {
                                                @Override
                                                public void run() {
                                                    Toast toast=Toast.makeText(getApplicationContext(),"修改活动信息成功！",Toast.LENGTH_LONG);
                                                    toast.setGravity(Gravity.CENTER,0,400);
                                                    toast.show();
                                                    //然后删除之前的图片
                                                    if(selectCover!=null && !"".equals(selectCover) && !"".equals(finalRemoteUrl)&&activityShow.getResourcesList()!=null && activityShow.getResourcesList().size()>0) {

                                                        DeleteObjectRequest delete = new DeleteObjectRequest(bucket, activityShow.getResourcesList().get(0).getAct_resource_url());
                                                        try {
                                ...
                                public void failed(Call call, IOException e) {
                                   ...
                                   runOnUiThread(new Runnable() {
                                    ...
                                    Toast toast=Toast.makeText(getApplicationContext(),"修改活动信息失败，发生未知异常..",Toast.LENGTH_LONG);
                                    toast.setGravity(Gravity.CENTER,0,400);
                                    toast.show();
                                    overridePendingTransition(0,R.anim.imap_fade_out_right);
                                    finish();
    ...
    public void makeSureUniversity(Integer uni_id){
        NetUtils netUtils=NetUtils.getInstance();
        String token=((AppContext)getApplicationContext()).getToken();
        Map map=new HashMap();
        map.put("uni_id", String.valueOf(uni_id));
        try {
            netUtils.postDataAsynToNetToken(AsyncHttpHelper.URL_UNIVERSITY + "/findUniById", token, map, new NetUtils.MyNetCall() {
                @Override
                public void success(Call call, Response response) throws IOException, JSONException {
                    String ret=response.body().string();
                    JSONObject json=new JSONObject(ret);
                    ...
                    if(json!=null && json.has("university")) {
                         jsonObject = json.getJSONObject("university");
                        University uni = gson.fromJson(jsonObject.toString(),University.class);
                        if(uni!=null&& !"".equals(uni)) {
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    university = uni.getUni_name();
                                    university_id=uni.getUni_id();
                                    tv_uni_text.setText(university);
                                }
                            });
                        }
        ...
        try {
            netUtils.postDataAsynToNetToken(AsyncHttpHelper.URL_PRO_CITY_AREA + "/findProByCityname", token, map, new NetUtils.MyNetCall() {
                ...
                public void success(Call call, Response response) throws IOException, JSONException {
                    ...
                    final Gson gson=new Gson();
                    if(json!=null && json.has("province_id") && json.has("province_name")) {
