...
import com.test.hyxc.chat.activity.ChatDetailActivity;
import com.test.hyxc.chat.activity.FriendInfoActivity;
import com.test.hyxc.chat.activity.GroupAvatarActivity;
import com.test.hyxc.chat.activity.GroupGridViewActivity;
import com.test.hyxc.chat.activity.GroupNotFriendActivity;
import com.test.hyxc.chat.activity.MainActivity;
import com.test.hyxc.chat.activity.MembersInChatActivity;
import com.test.hyxc.chat.activity.PersonalActivity;
import com.test.hyxc.chat.activity.VerificationActivity;
import com.test.hyxc.chat.activity.historyfile.activity.HistoryFileActivity;
import com.test.hyxc.chat.adapter.GroupMemberGridAdapter;
...
import com.test.hyxc.chat.view.ChatDetailView;
import com.test.hyxc.chat.view.SlipButton;
...
public class ChatDetailController implements OnClickListener, OnItemClickListener,
    ...
    public void initData() {
        ...
        mGroupId = intent.getLongExtra(AppContext.GROUP_ID, 0);
        mTargetId = intent.getStringExtra(AppContext.TARGET_ID);
        mTargetAppKey = intent.getStringExtra(AppContext.TARGET_APP_KEY);
        UserInfo myInfo = JMessageClient.getMyInfo();
        mMyNickName = myInfo.getNickname();
        mMyUsername = myInfo.getUserName();
        // 是群组
        if (mGroupId != 0) {
            mChatDetailView.setTitle("海岛消息设置");
            mIsGroup = true;
           ...
           /* if (mGroupInfo.getAvatarFile() != null && mGroupInfo.getAvatarFile().exists()) {
            ...
            mChatDetailView.setGridHide(mGroupInfo);
            //initAdapter();
    ...
    private void initAdapter() {
        // 初始化头像
        mGridAdapter = new GroupMemberGridAdapter(mContext,mGroupInfo,1);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            case R.id.return_btn:
                ...
                intent.putExtra(AppContext.CONV_TITLE, getName());
                intent.putExtra(AppContext.MEMBERS_COUNT, mMemberInfoList.size());
                mContext.setResult(AppContext.RESULT_CODE_CHAT_DETAIL, intent);
                mContext.finish();
                break;
            ...
            case R.id.rl_groupAvatar:
                ...
                break;
            case R.id.iv_group_logo:
                Intent intent2=new Intent();
                intent2.putExtra("groupId",mGroupId);
                intent2.setClass(mContext, IslandShowActivity.class);
                mContext.startActivity(intent2);
                mContext.finish();
                break;
            //去主页
            case R.id.goto_inside:
                if(!mIsGroup) { //单聊
                    Intent intent1 = new Intent();
                    intent1.putExtra("userName", mUserInfo.getUserName());
                    ...
                    intent1.setClass(mContext, IslandShowActivity.class);
                    mContext.startActivity(intent1);
                    mContext.finish();
                }
                break;
            ...
            case R.id.tv_moreGroup:
                ...
                intent.putExtra(AppContext.DELETE_MODE, false);
                mContext.startActivityForResult(intent, AppContext.REQUEST_CODE_ALL_MEMBER);
                break;
                                        ...
                                        if (content.getContentType() == ContentType.image) {
                                            ...
                                            if (!TextUtils.isEmpty(localPath)) {
                                                ...
                                                if (file.exists()) {
                                                    ...
                                                    File copyFile = new File(AppContext.FILE_DIR + fileContent.getFileName());
    ...
    public void onItemClick(AdapterView<?> viewAdapter, View view, final int position, long id) {
        ...
        if (mIsGroup) {
            ...
            if (position < mCurrentNum) {
                ...
                intent.putExtra(AppContext.DELETE_MODE, true);
                intent.putExtra(AppContext.GROUP_ID, mGroupId);
            ...
            mContext.startActivityForResult(intent, AppContext.REQUEST_CODE_FRIEND_INFO);*/
        } else if (position == mCurrentNum) {
