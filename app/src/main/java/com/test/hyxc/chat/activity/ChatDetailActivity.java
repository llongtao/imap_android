...
import com.test.hyxc.chat.controller.ChatDetailController;
...
public class ChatDetailActivity extends BaseActivity {
    ...
    public void onBackPressed() {
        ...
        intent.putExtra(AppContext.CONV_TITLE, mChatDetailController.getName());
        intent.putExtra(AppContext.MEMBERS_COUNT, mChatDetailController.getCurrentCount());
        intent.putExtra("deleteMsg", mChatDetailController.getDeleteFlag());
        setResult(AppContext.RESULT_CODE_CHAT_DETAIL, intent);
        finish();
        super.onBackPressed();
    }
            ...
            switch (requestCode) {
                ...
                case MY_NAME_REQUEST_CODE:
                    if (data.getBooleanExtra("returnChatActivity", false)) {
                        ...
                        data.putExtra(AppContext.NAME, mChatDetailController.getName());
                        setResult(AppContext.RESULT_CODE_CHAT_DETAIL, data);
                        finish();
                    }
                    break;
                case AppContext.REQUEST_CODE_ALL_MEMBER:
    ...
    public void startChatActivity(long groupID, String groupName, int member) {
        ...
        intent.putExtra(AppContext.MEMBERS_COUNT, member + 1);
        intent.putExtra(AppContext.GROUP_ID, groupID);
        intent.putExtra(AppContext.CONV_TITLE, groupName);
    ...
    public void onEvent(MessageEvent event) {
        ...
        if (msg.getContentType() == ContentType.eventNotification) {
            ...
            handleMsg.what = AppContext.ON_GROUP_EVENT;
            Bundle bundle = new Bundle();
            bundle.putLong(AppContext.GROUP_ID, ((GroupInfo) msg.getTargetInfo()).getGroupID());
    ...
    private static class UIHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case AppContext.ON_GROUP_EVENT:
                        activity.mChatDetailController.refresh(msg.getData()
                                .getLong(AppContext.GROUP_ID, 0));
                        break;
                }
            }
        }
