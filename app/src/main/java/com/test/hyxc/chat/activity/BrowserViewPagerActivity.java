...
import com.test.hyxc.chat.pickerimage.utils.AttachmentStore;
import com.test.hyxc.chat.pickerimage.utils.StorageUtil;
import com.test.hyxc.chat.utils.BitmapLoader;
import com.test.hyxc.chat.utils.DialogCreator;
import com.test.hyxc.chat.utils.NativeImageLoader;
import com.test.hyxc.chat.view.ImgBrowserViewPager;
import com.test.hyxc.chat.view.PhotoView;
...
public class BrowserViewPagerActivity extends BaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        long groupId = intent.getLongExtra(AppContext.GROUP_ID, 0);
        String targetAppKey = intent.getStringExtra(AppContext.TARGET_APP_KEY);
        mMessageId = intent.getIntExtra("msgId", 0);
        if (groupId != 0) {
            mConv = JMessageClient.getGroupConversation(groupId);
        } else {
            String targetId = intent.getStringExtra(AppContext.TARGET_ID);
            if (targetId != null) {
                mConv = JMessageClient.getSingleConversation(targetId, targetAppKey);
            }
        }
        mStart = intent.getIntExtra("msgCount", 0);
        mPosition = intent.getIntExtra(AppContext.POSITION, 0);
                    ...
                    public void onClick(View v) {
                        switch (v.getId()) {
                            ...
                            case R.id.jmui_top_conv_ll://转发
                                ...
                                AppContext.forwardMsg.add(mMsg);
...
//        }
    ...
    private void initImgPathList() {
        mMsgIdList = this.getIntent().getIntegerArrayListExtra(AppContext.MsgIDs);
        Message msg;
        ImageContent ic;
        ...
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.return_btn:
                    ...
                    setResult(AppContext.RESULT_CODE_SELECT_PICTURE, intent);
                    finish();
                    break;
                ...
                imgContent.downloadOriginImage(mMsg, new DownloadCompletionCallback() {
                    ...
                    public void onComplete(int status, String desc, File file) {
                        ...
                        if (status == 0) {
                            ...
                            bundle.putInt(AppContext.POSITION, mViewPager.getCurrentItem());
                            msg.setData(bundle);
                            msg.sendToTarget();
                        } else {
                            if (mProgressDialog != null) {
            ...
            if (activity != null) {
                switch (msg.what) {
                    case DOWNLOAD_ORIGIN_IMAGE_SUCCEED:
                        ...
                        activity.mPathList.set(bundle.getInt(AppContext.POSITION), bundle.getString("path"));
                    ...
                    case SEND_PICTURE:
                        Intent intent = new Intent();
                        intent.putExtra(AppContext.MsgIDs, activity.mMsgIds);
                        activity.setResult(AppContext.RESULT_CODE_BROWSER_PICTURE, intent);
                        activity.finish();
                        break;
                    //显示下载原图进度
