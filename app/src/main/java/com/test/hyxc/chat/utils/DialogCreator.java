...
//                        intent.setClass(context, ResetPasswordActivity.class);
    ...
    public static void createForwardMsg(final Context context, int mWidth, final boolean isSingle, final Conversation conv,
        ...
        final Message message = AppContext.forwardMsg.get(0);
        ...
        switch (message.getContentType()) {
            ...
            case location:
                ...
                break;
            case custom:
                String work_text = ((CustomContent) message.getContent()).getStringValue("work_text");
                String act_title = ((CustomContent)message.getContent()).getStringValue("act_title");
                String is_name = ((CustomContent)message.getContent()).getStringValue("is_name");
                if(work_text!=null && !"".equals(work_text)) {
                    content.setVisibility(View.VISIBLE);
                    content.setText("[作品]" + (work_text.length()>8?work_text.substring(0,7)+"...":work_text));
                    break;
                }
                if(act_title!=null && !"".equals(act_title)){
                    content.setVisibility(View.VISIBLE);
                    content.setText("[活动]" + (act_title.length()>8?act_title.substring(0,7)+"...":act_title));
                    break;
                }
                if(is_name!=null && !"".equals(is_name)){
                    content.setVisibility(View.VISIBLE);
                    content.setText("[海岛分享]"+ is_name);
                    break;
                }
        ...
        commit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                //对于作品分享类的转发 使用
                if(message.getContentType().equals(ContentType.custom) && ((CustomContent) message.getContent()).getStringValue("share_type")!=null && "work".equals(((CustomContent) message.getContent()).getStringValue("share_type"))){
                    ...
                    message.setOnSendCompleteCallback(new BasicCallback() {
                        ...
                        public void gotResult(int responseCode, String responseDesc) {
                            if (responseCode == 0) {
                                ...
                                //修改作品的转发数量+1///////////
                                Integer work_id = Integer.parseInt(((CustomContent) message.getContent()).getStringValue("work_id"));
                                changeWorkShareCount(work_id,context);
                                SharePreferenceManager.setIsOpen(true);
                                ActivityController.finishAll();
                            } else if(responseCode == 871302){
                                mLoadingDialog.dismiss();
                                dialog.dismiss();
                                //消息发送成功
                                Toast.makeText(context, "消息过长，发送失败~", Toast.LENGTH_SHORT).show();
                                SharePreferenceManager.setIsOpen(true);
                                ActivityController.finishAll();
                            }else {
                                //消息发送失败
                ...
                }else if(message.getContentType().equals(ContentType.custom) && ((CustomContent) message.getContent()).getStringValue("share_type")!=null && "activity".equals(((CustomContent) message.getContent()).getStringValue("share_type"))){
                    MessageSendingOptions options = new MessageSendingOptions();
                    options.setNeedReadReceipt(true);
                    message.setOnSendCompleteCallback(new BasicCallback() {
                        @Override
                    ...
                    JMessageClient.sendMessage(message,options);
                }else if(message.getContentType().equals(ContentType.custom) && ((CustomContent) message.getContent()).getStringValue("share_type")!=null && "island".equals(((CustomContent) message.getContent()).getStringValue("share_type"))){
                    MessageSendingOptions options = new MessageSendingOptions();
                    options.setNeedReadReceipt(true);
                    message.setOnSendCompleteCallback(new BasicCallback() {
                        @Override
    ...
    public static void changeWorkShareCount(int work_id, Context context){
        ...
        try {
            netUtils.postDataAsynToNetToken(AsyncHttpHelper.URL_WORK_BASE + "/shareCountAddOne", token, map, new NetUtils.MyNetCall() {
                @Override
                public void success(Call call, Response response) throws IOException, JSONException {
                    ...
                    if (json.has("tokenState") && json.getString("tokenState").equals("tokenTimeout")) {
                        ...
                        String result = json.getString("shareState");
                    }
                }
                ...
                public void failed(Call call, IOException e) {
                    activity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast toast = Toast.makeText(activity, "修改转发数量异常~", Toast.LENGTH_SHORT);
