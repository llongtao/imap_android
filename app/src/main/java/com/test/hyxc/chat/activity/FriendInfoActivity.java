...
import com.test.hyxc.chat.controller.FriendInfoController;
...
import com.test.hyxc.chat.view.FriendInfoView;
...
public class FriendInfoActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mTargetId = getIntent().getStringExtra(AppContext.TARGET_ID);
        mTargetAppKey = getIntent().getStringExtra(AppContext.TARGET_APP_KEY);
        ...
        if (mIsFromContact || mIsFromSearch || mFromGroup || mIsAddFriend) {
            ...
            mGroupId = getIntent().getLongExtra(AppContext.GROUP_ID, 0);
            Conversation conv;
            if (mGroupId == 0) {
    ...
    public void startChatActivity() {
        if (mIsFromContact || mIsAddFriend || mIsFromSearch || mFromGroup) {
            ...
            intent.putExtra(AppContext.CONV_TITLE, title);
            intent.putExtra(AppContext.TARGET_ID, mUserInfo.getUserName());
            intent.putExtra(AppContext.TARGET_APP_KEY, mUserInfo.getAppKey());
            ...
            if (mGroupId != 0) {
                ...
                intent.putExtra(AppContext.CONV_TITLE, mTitle);
                setResult(AppContext.RESULT_CODE_FRIEND_INFO, intent);
            }
        }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == AppContext.RESULT_CODE_EDIT_NOTENAME) {
            mTitle = data.getStringExtra(AppContext.NOTENAME);
            FriendEntry friend = FriendEntry.getFriend(AppContext.getUserEntry(), mTargetId, mTargetAppKey);
            if (null != friend) {
