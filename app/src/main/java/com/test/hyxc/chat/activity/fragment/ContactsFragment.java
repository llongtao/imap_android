package com.test.hyxc.chat.activity.fragment;
...
import com.test.hyxc.chat.controller.ContactsController;
...
import com.test.hyxc.chat.utils.pinyin.HanziToPinyin;
import com.test.hyxc.chat.view.ContactsView;
...
public class ContactsFragment extends BaseFragment {
    ...
    public void onEvent(ContactNotifyEvent event) {
        final UserEntry user = AppContext.getUserEntry();
        ...
        if (event.getType() == ContactNotifyEvent.Type.invite_accepted) {
            ...
            AppContext.forAddFriend.remove(username);
            FriendRecommendEntry entry = FriendRecommendEntry.getEntry(user, username, appKey);
            ...
            if (AppContext.forAddFriend.size() > 0) {
                for (String forAdd : AppContext.forAddFriend) {
                    if (forAdd.equals(username)) {
                        return;
                    } else {
                        AppContext.forAddFriend.add(username);
                    }
                }
            } else {
                AppContext.forAddFriend.add(username);
