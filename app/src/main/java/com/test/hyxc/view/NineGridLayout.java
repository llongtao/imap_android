...
public abstract class NineGridLayout extends ViewGroup {
    ...
    private List<WorkResource> mUrlList = new ArrayList<>();
    WorkShow workShow;
    int type=0;
    public NineGridLayout(Context context) {
        super(context);
        init(context);
        hosturl=((AppContext)context.getApplicationContext()).getOssConfig().getHost()+"/";
    }
    ...
    public void setUrlList(List<WorkResource> urlList) {
    ...
    private void refresh() {
        ...
        if (size == 1) {
            String url=hosturl+mUrlList.get(0).getWork_resource_url();
            if(type==1) {
                url=hosturl+mUrlList.get(0).getWork_resource_url()+"?x-oss-process=video/snapshot,t_1000,f_jpg,w_0,h_0,m_fast";
            }
            RatioImageView imageView = createImageView(0, url,type);
        ...
        for (int i = 0; i < size; i++) {
            String url =hosturl+mUrlList.get(i).getWork_resource_url();
            if(type==1){
                url=hosturl+mUrlList.get(i).getWork_resource_url()+"?x-oss-process=video/snapshot,t_1000,f_jpg,w_0,h_0,m_fast";
            }
            RatioImageView imageView;
            if (!mIsShowAll) {
                if (i < MAX_COUNT - 1) {
                    imageView = createImageView(i, url,type);
    ...
    private RatioImageView createImageView(final int i, final String url,int type) {
        RatioImageView imageView = new RatioImageView(mContext,type);
        imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
        imageView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                onClickImage(i, url, mUrlList,workShow);
            }
        });
        return imageView;
    }
...
//        params.width = width;
    ...
    private int getListSize(List<WorkResource> list) {
    ...
    protected abstract void onClickImage(int position, String url, List<WorkResource> urlList,WorkShow workShow);
    ...
    public void setWorkShow(WorkShow workShow) {
        ...
        List< WorkResource> lw= workShow.getResourcesList();
        setUrlList(lw);
    }
}
