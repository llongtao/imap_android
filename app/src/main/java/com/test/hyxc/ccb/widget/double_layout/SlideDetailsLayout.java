package com.test.hyxc.ccb.widget.double_layout;
...
public class SlideDetailsLayout extends ViewGroup {
    ...
    protected void onFinishInflate() {
        ...
        if (mDefaultPanel == 1) {
         ...
         //如果第一层布局的显示状态是Gone，则显示第二层
        if (mFrontView.getVisibility() == GONE)  post(new Runnable() {
            @Override
            public void run() {
                smoothOpen(false);
            }
        });
        //如果第二层布局的显示状态是Gone，则显示第一层
        if (mBehindView.getVisibility() == GONE)  post(new Runnable() {
            @Override
            public void run() {
                smoothClose(false);
            }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (mFrontView.getVisibility() == GONE || mBehindView.getVisibility() == GONE) {
            //如果双层布局中任意一层被Gone，则不可切换上下层
